...
Params_Healthy = [];
dataset_list = ['AML_02_1.mat'; 'AML_02_2.mat';'AML_02_3.mat'];
...
for idx = 1:3
    ...
    Rtoe = Markers.RTOE(:,3)- mean(Markers.RTOE(:,3));
    Ltoe = Markers.LTOE(:,3) - mean(Markers.LTOE(:,3));
    Rank = Markers.RANK(:,3)- mean(Markers.RANK(:,3));
    Lank = Markers.LANK(:,3)- mean(Markers.LANK(:,3));
    Rhip = Markers.RHIP(:,3)- mean(Markers.RHIP(:,3));
    Lhip = Markers.LHIP(:,3)- mean(Markers.LHIP(:,3));
    ...
    Params_Healthy.(name).Rdiv = Rdivision;
    Params_Healthy.(name).Ldiv = Ldivision;
...
save('params_Healthy.mat','Params_Healthy')
